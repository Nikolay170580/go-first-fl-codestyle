...
func startTraining(charName, charClass string) string {
	if charClass == "warrior" {
		return fmt.Printf("%s, ты Воитель - отличный боец ближнего боя.\n", charName)
	}

	if charClass == "mage" {
		return fmt.Printf("%s, ты Маг - превосходный укротитель стихий.\n", charName)
	}
...
func choiseCharClass() string {
	...
	for approveСhoice != "y" {
		...
		fmt.Scanf("%s\n", &approveChoice)
		approveСhoice = strings.ToLower(approveChoice)
	}
	return charClass
}
